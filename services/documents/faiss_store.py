...
class FAISSStore:
    ...
    @classmethod
    def load(cls, directory: str) -> "FAISSStore | None":
    ...
    def search(self, query_embedding: np.ndarray, k: int = 5, source_filter: str = None) -> list[str]:
        ...
        if source_filter and not results:
            # Expand search
            expanded_k = min(50, self.index.ntotal)
            _, indices = self.index.search(np.array(query_embedding, dtype=np.float32), expanded_k)
            for i in indices[0]:
                if i < 0:
                    continue
                doc = self.documents[i]
